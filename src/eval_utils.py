...
def predict_sl_values(dataloader, model):
    ...
    pool_dict = {}
    with torch.no_grad():
        for i, (toks, lengths, np_mask, targets, targets_seq, labels) in tqdm.tqdm(
            enumerate(dataloader)
        ):
            with torch.autocast(device_type=device, dtype=dtype):
                y_pred, y_pool, y_attn = model.predict(
                    toks.to(device), lengths.to(device), np_mask.to(device)
                )
            ...
            for j in range(len(labels)):
                if len(labels) == 1:
                    ...
                    annot_dict[labels[j]] = (
                        y_attn[j, : lengths[j]].float().cpu().numpy()
                    )
...
def generate_sl_outputs(
    ...
    for outer_i in range(1):
        ...
        if not os.path.exists(
            os.path.join(
                model_attrs.outputs_save_path, f"inner_{outer_i}_{inner_i}.pkl"
            )
        ):
...
def generate_ss_outputs(
    ...
    for outer_i in range(5):
        print("Generating output for ensemble model", outer_i)
        X_train, y_train, X_test, y_test = datahandler.get_swissprot_ss_xy(
        ...
        for type_i in range(9):
            thresh[type_i] = get_best_threshold_mcc(
                y_train[:, type_i], y_train_preds[:, type_i]
            )
        ...
        pickle.dump(
            y_test_preds,
            open(f"{model_attrs.outputs_save_path}/ss_{outer_i}.pkl", "wb"),
        )

    with open(
        os.path.join(model_attrs.outputs_save_path, f"thresholds_ss_mcc.pkl"), "wb"
    ) as f:
        pickle.dump(threshold_dict, f)
