...
def focal_loss(input, target, gamma=1):
    bceloss = F.binary_cross_entropy_with_logits(
        input, target, pos_weight=pos_weights_bce.to(input.device), reduction="none"
    )
...
class AttentionHead(nn.Module):
    ...
    def forward(self, x, np_mask, lengths):
        ...
        x = self.preattn_ln(x)
        mul = (x * self.Q.weight.view(1, 1, n_heads, hidden_dim // n_heads)).sum(
            -1
        )  # * np.sqrt(5)
        ...
        for i in range(mul.size(0)):
            ...
            mul_score_list.append(
                F.pad(
                    smooth_tensor_1d(mul[i, : lengths[i], 0].unsqueeze(0), 2).unsqueeze(
                        0
                    ),
...
class BaseModel(pl.LightningModule):
    def __init__(self, embed_dim) -> None:
        ...
        # self.relu1 = nn.Dropout(0.4)
        # self.lin2 = nn.Linear(512, 64)
        # self.relu2 = nn.Dropout(0.1)
        # self.attn_head1 = AttentionHead(64, 1)
        self.clf_head = nn.Linear(embed_dim, 11)
    ...
    def attn_reg_loss(self, y_true, y_attn, y_tags, lengths, n):
        ...
        for i in range(y_true.size(0)):
            reg_loss += fourier_att_prior_loss_dct(
                F.pad(
                    y_attn[i, : lengths[i]].unsqueeze(0).unsqueeze(0),
                    (8, 8),
                    "replicate",
                ).squeeze(1),
                lengths[i] // 6,
                ...
                for j in range(9):
                    if (j + 1) in y_tags[i]:
                        ...
                        kld_loss += pos_weights_annot[j] * self.kld(
                            ...
                            pos_tar[: lengths[i]].unsqueeze(0)
                            / pos_tar[: lengths[i]].sum().unsqueeze(0),
                        )
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(grouped_parameters, lr=self.lr, weight_decay=1e-3)
...
class SignalTypeMLP(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = nn.BCEWithLogitsLoss(pos_weight=pos_weights_annot.to(y_pred.device))(
            y_pred, y
        )
        self.log("train_loss_batch", loss, on_epoch=True)
        return {"loss": loss}
