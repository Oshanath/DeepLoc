...
def get_optimal_threshold_pr(output_df, data_df):
    ...
    optimal_thresholds = np.zeros((11,))
    for i in range(11):
        pr, re, thresholds = metrics.precision_recall_curve(
...
def calculate_sl_metrics_fold(test_df, thresholds):
    ...
    metrics_dict["Jaccard_subloc"] = jaccard_score(
        y_subloc, ypred_subloc, average="samples"
    )
    ...
    for i in range(10):
        metrics_dict[f"{CATEGORIES[1+i]}"] = matthews_corrcoef(
            y_subloc[:, i], ypred_subloc[:, i]
        )
...
def calculate_sl_metrics(
    ...
    with open(
        os.path.join(model_attrs.outputs_save_path, f"thresholds_sl_{thresh_type}.pkl"),
        "rb",
    ) as f:
    ...
    for k in metrics_dict_list:
        output_dict[k] = [
            f"{round(np.array(metrics_dict_list[k]).mean(), 2):.2f} pm {round(np.array(metrics_dict_list[k]).std(), 2):.2f}"
        ]
    ...
    for k in metrics_dict_list:
        print(
            "{0:21s} : {1}".format(
                k,
                f"{round(np.array(metrics_dict_list[k]).mean(), 2):.2f} + {round(np.array(metrics_dict_list[k]).std(), 2):.2f}",
            )
        )
    for k in metrics_dict_list:
        print(
...
def calculate_ss_metrics_fold(y_test, y_test_preds, thresh):
    ...
    for j in range(len(SS_CATEGORIES) - 1):
        metrics_dict[f"{SS_CATEGORIES[j+1]}"] = matthews_corrcoef(
            y_preds[:, j], y_test[:, j]
        )
...
def calculate_ss_metrics(
    model_attrs: ModelAttributes, datahandler: DataloaderHandler, thresh_type="mcc"
):
    with open(
        os.path.join(model_attrs.outputs_save_path, f"thresholds_ss_{thresh_type}.pkl"),
        "rb",
    ) as f:
        threshold_dict = pickle.load(f)
    # print(np.array(list(threshold_dict.values())).mean(0))
    ...
    for outer_i in range(5):
        _, _, _, y_test = datahandler.get_swissprot_ss_xy(
            model_attrs.outputs_save_path, outer_i
        )
        y_test_preds = pickle.load(
            open(f"{model_attrs.outputs_save_path}/ss_{outer_i}.pkl", "rb")
        )
